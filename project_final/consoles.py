...
class FatTreeTopology(Topo):
    ...
    def __init__(self, core=2, spine=4, host_rer_leaf=2):
        Topo.__init__(self)
        ...
        # Create hosts
        hosts = []
        for i in range(host_rer_leaf * leaf):
        ...
        # Spine to leaf edges
        leaf_per_spine = leaf / spine
        direction = 0  # 0 - forward, 1 - backward
        for i in range(spine):
            for j in range(spine_to_leaf):
                if spine_to_leaf == leaf_per_spine:
                    self.addLink(spines[i], leaves[i * spine_to_leaf + j])
                elif spine_to_leaf > leaf_per_spine:
                    self.addLink(spines[i], leaves[(i - (i % leaf_per_spine)) * leaf_per_spine + j])
            direction = 1 - direction

        # Leaf to host edges
        for i in range(leaf):
            for j in range(host_rer_leaf):
                self.addLink(leaves[i], hosts[i * host_rer_leaf + j])
...
class Graph(Frame):
    ...
    def __init__(self, parent=None, bg='white', gheight=500, gwidth=500,
                 barwidth=10, ymax=50, ):
...
class ConsoleApp(Frame):
    ...
    def iperf(self):
        ...
        for console in consoles:
            i = ( i + 1 ) % count
            ip = consoles[i].node.IP()
            console.sendCmd('iperf -t 15 -i 1 -c ' + ip)
...
if __name__ == '__main__':
    ...
    # create the network with the needed topology
    print "Creating Mininet network..."
    net = Mininet(topo=topology, controller=None)

    # assign the controller to the network
    print "Connecting to remote pox controller 127.0.0.1@6633..."

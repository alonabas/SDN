...
class Tutorial (object):
    ...
    def __init__(self, connection):
        ...
        self.mac_table = {}  # create mac table
    ...
    def delete_flow_mod_by_port(self, port):
        ...
        action = of.ofp_action_output(port=port) # delete as destination port
    ...
    def delete_flow_mod_by_port_in(self, port):
        log.debug('Delete flows: (Switch %d, Port %d) -> any' % (self.connection.dpid,port))
        fm = of.ofp_flow_mod()
        # rule match
        fm.match = of.ofp_match()
    ...
    def delete_flow_mod_by_mac_dst(self,mac_addr):
        """
        Delete flows from switch by destination mac address
        """
        log.debug('Delete rules: any -> MAC_dst %s' % (str(mac_addr)))
        # rule match
        fm = of.ofp_flow_mod()
        # delete rules for given when destination is given source MAC that changed its location
        fm.match = of.ofp_match()
        # delete source
        fm.match.dl_src = mac_addr
        # delete flow mod that match pattern
        fm.command = of.OFPFC_DELETE
        # Send message to switch
        self.connection.send(fm)
        ...
        fm.match.dl_dst = mac_addr
        # delete flow mod that match pattern
        fm.command = of.OFPFC_DELETE
        # Send message to switch
        for connection in core.openflow.connections:  # send to all connections
            connection.send(fm)
    ...
    def act_like_switch(self, packet, packet_in):
        ...
        # Check if the source changed it's location
        ...
        # if packet.dst not in mac table - flood
        if packet.dst not in self.mac_table:
            log.debug('MAC %s is not in mac table. Flood packet.' % (str(packet.dst)))
            self.send_packet(packet_in.buffer_id, packet_in.data, of.OFPP_FLOOD, packet_in.in_port)
        else:
            # if packet.dst is in table send to table[packet.dst]            
            # install rule
